...
public class RangeOption extends AbstractOption<Integer> implements IRangeOption<Integer> {
	...
	public static class Builder {
		...
		public RangeOption build(Function<ModConfig, ForgeConfigSpec.ConfigValue<Integer>> cache) {
	...
	public static class Widget<T extends Number, O extends IOption<T> & IRangeOption<T>> extends OptionWidget<T, O> {
		...
		protected void drawCompact(PoseStack matrixStack, int mouseX, int mouseY, float partialTicks) {
			...
			this.drawText(matrixStack, valueText, drawX, drawY, this.getTextColor());
		}
		...
		protected void drawThumb(PoseStack matrixStack, int mouseX, int mouseY, float partialTicks) {
			double thumbOffset = Mth.clamp((this.modifiableValue.doubleValue() - this.option.getMinValue().doubleValue())
							/ (this.option.getMaxValue().doubleValue() - this.option.getMinValue().doubleValue()) * this.getSliderSize(),
		...
		private void setValue(double newValue) {
			...
			double value = this.option.getMinValue().doubleValue()
					+ valuePercent * (this.option.getMaxValue().doubleValue() - this.option.getMinValue().doubleValue());
		...
		private double getThumbPos() {
			double value = Mth.clamp(this.modifiableValue.doubleValue(),
					this.option.getMinValue().doubleValue(), this.option.getMaxValue().doubleValue());
			double interval = this.option.getInterval().doubleValue();

			return Math.ceil(value / interval) * interval
					/ (this.option.getMaxValue().doubleValue() - this.option.getMinValue().doubleValue());
		}

		protected int getSliderSize() {
			return (int) (this.fadingDepth * 90);
		}
